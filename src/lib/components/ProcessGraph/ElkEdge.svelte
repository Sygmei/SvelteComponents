<script lang="ts">
    ...
    function getPath(): string {
        ...
        const confluencePoints = get(confluencePointsStore);

        const boxMap = new Map<string, GroupBox>();
        boxes.forEach((b) => boxMap.set(b.id, b));

        // Get confluence points for source and target
        const sourceConfluence = confluencePoints.get(source);
        ...
        // Get current position after exit waypoints
        const lastExitWaypoint = waypoints[waypoints.length - 1];
        let effectiveSourceY = lastExitWaypoint.y;
        let effectiveSourceX = lastExitWaypoint.x;

        // If source has a branch point (multiple outgoing edges), go to branch Y
        // This ensures all edges from the same source branch at the same Y position
        if (sourceConfluence?.branchY !== undefined) {
            const branchY = sourceConfluence.branchY;
            // Only add branch point if it's below the current position and above target
            if (branchY > effectiveSourceY && branchY < targetY) {
                waypoints.push({ x: effectiveSourceX, y: branchY });
                effectiveSourceY = branchY;
            }
        }
        ...
        } else if (finalTurnY < effectiveSourceY) {
            // We need to go UP to the join point - go horizontal first at current Y, then up
            waypoints.push({ x: targetX, y: effectiveSourceY });
            waypoints.push({ x: targetX, y: finalTurnY });
            // Skip the normal horizontal movement since we already did it
            waypoints.push({ x: targetX, y: targetY });
            return buildPathFromWaypoints(waypoints);
        }
    ...
    function buildAvoidancePath(
        ...
        if (joinY !== undefined && joinY < tgtY) {
            ...
            if (joinY > currentY) {
                // Go down to join Y first (vertical)
                waypoints.push({ x: currentX, y: joinY });
                // Move horizontally to target X at join Y
                if (currentX !== tgtX) {
                    waypoints.push({ x: tgtX, y: joinY });
                }
            } else if (joinY < currentY) {
                // We're already below joinY - go horizontal first, then up to joinY
                if (currentX !== tgtX) {
                    waypoints.push({ x: tgtX, y: currentY });
                }
                waypoints.push({ x: tgtX, y: joinY });
            } else {
                // We're exactly at joinY - just go horizontal
                if (currentX !== tgtX) {
                    waypoints.push({ x: tgtX, y: joinY });
                }
            }
        } else {
            // No join point - regular connection (horizontal then vertical)
            if (currentX !== tgtX) {
                waypoints.push({ x: tgtX, y: currentY });
            }
        }
